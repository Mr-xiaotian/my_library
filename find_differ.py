...
def fd_dict(dict_a,dict_b):
    ...
    for key in key_min:
        if dict_a[key] == dict_b[key]:
            ...
            print(key, ':')
            compare_strings(dict_a[key],dict_b[key])
            print()
...
def compare_strings(str1: str, str2: str) -> None:
    """
    ...
    print_diffs(str1, diff_ranges)
    print_diffs(str2, diff_ranges)

def print_diffs(input_str: str, diff_ranges: list, 
                nor_end = '\033[0m', dif_end = '\033[1m') -> None:
    """
    ...
    nor_end: can use '\033[0m' '_' or others.
    dif_end: can use '\033[1m' '_' or others.
    """

    prev_end_index = 0
    for start_index, end_index in diff_ranges:
        end_index = min(end_index, len(input_str))
        print(input_str[prev_end_index:start_index], end = nor_end)
        print(input_str[start_index:end_index], end = dif_end)
